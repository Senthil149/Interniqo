  ...
  return (
    <div className="relative inline-block text-left" ref={popoverRef}>
      <button
        ...
        className={`inline-flex items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-semibold shadow-xs transition-colors cursor-pointer ${config.badgeClass}`}
      >
        ...
        {interactive && (
          <span className="text-[10px] opacity-60 ml-0.5">ℹ️</span>
        )}
      </button>
      ...
      {open && (
        <div
          ...
          className="absolute z-30 mt-2 w-80 sm:w-96 right-0 rounded-xl border border-slate-200 bg-white p-4 shadow-xl text-left focus:outline-none"
        >
          {/* Header */}
          <div className={`-m-4 mb-3 p-3.5 rounded-t-xl border-b flex items-center justify-between ${config.headerClass}`}>
          ...
          <div className="mt-2 space-y-2 max-h-48 overflow-y-auto pr-1">
            ...
            {loading ? (
              ...
              <p className="text-xs text-slate-600 bg-emerald-50/60 border border-emerald-100 p-2 rounded-lg leading-relaxed">
          ...
          <div className="mt-3 pt-2.5 border-t border-slate-100 text-[11px] text-slate-500 leading-normal flex items-start gap-1.5">
            <span className="text-slate-400 flex-shrink-0 text-xs">ℹ️</span>
            <span>
